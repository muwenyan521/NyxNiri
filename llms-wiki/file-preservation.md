...
## 机制 2：manifest preserve（按声明快照）

`.module.toml` 的 `preserve = ["monitor.kdl"]` 声明要保留的文件（按**名**，不是魔法文件名）。
`_phase_atomic_deployment` 在 `atomic_replace` **前**用 `_snapshot_preserved` 把这些文件
`copy2` 到 tempfile 快照，deploy **后** `_restore_preserved` 复制回 dest。
